...
//#define randomS(a,b) (((double)rand()/RAND_MAX)*(b-a)+a)
...
void DDPG_C(float ab_, float v_, float asip_, float u1_)
{
	ab = ab_;
	v = v_;
	asip = asip_;
...
double rand_normal(double mu, double sigma) {
	double z = sqrt(-2.0*log(Uniform())) * sin(2.0*M_PII*Uniform());
	return mu + sigma * z;
}
...
void updatestate(states *state, int reset) {
	...
	*/
	...
	for (int i = 0; i < state->size; i++) {
		...
		x += v * 0.01;
		state->n_seq[i] = x;
		printf("state->action[]:%f\n", state->action[i]);
		printf("state->n_seq:%f\n", x);
	}*/
	
	double *ffv = getValue();
	for (int i = 0; i < state->size; i++) {

		state->n_seq[i] = ffv[i] * state->action[i];
		//printf("state->action[]:%f\n", state->action[i]);
	}
...
void get_action(network *p, states state, double *seq) {
	...
	for (int i = 0; i < p->size[p->len]; i++) {
		...
		//printf("uniform:%f\n", (Uniform()*1));
		//state.action[i] = fmax(fmin(tmp + (Uniform()*alpha * 4 - alpha), 1.0), -1.0);
		double ranV = randomS(4, 8);
		double ran = (ranV - 4) / 4;
		state.action[i] = ran;
		printf("state.action %f\n", state.action[i]);
