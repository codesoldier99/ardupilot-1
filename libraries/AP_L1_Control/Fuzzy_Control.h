...
public:
    ...
    float defuzzCentroid(int u_index[9], float u_u[9]);
    float defuzzMeanOfMax(int u_index[], float u_u[]);
    float defuzzSmallestOfMax(int u_index[], float u_u[]);
    float defuzzLargestOfMax(int u_index[], float u_u[]);
};
...
//面积中心法
float Fuzzy_controller::defuzzCentroid( int u_index[9], float u_u[9])
{
    float sum = 0,out=0;
    ...
    if (total_mf >= 0 && total_mf <= 0) total_mf = 1;
    out = sum / total_mf;
    if (out < umin) out = umin;
    else if (out > umax) out = umax;
    cout<< "   out  " << out << endl;
    return out;
}

//最大隶属度平均值法
float Fuzzy_controller::defuzzMeanOfMax(int u_index[], float u_u[])
{
    float step = fabs(umax - umin) / (2 * (N / 2));
    float num_index[9];
    float most = 0;
    float out = 0, sum = 0;
    float number[9];
    ...
    for (int i = 0; i < 9; i++)
    {
        num_index[i] = u_index[i] * step + (umax + umin) / 2;
        a_a[i] = fabs(u_u[i]);
    }

    most = *max_element(a_a, a_a + 9);
    cout << most << endl;

    for (int i = 0; i < 9; i++)
    {
        if (a_a[i] >= most && a_a[i] <= most)
        {
            number[j] = num_index[i];
            sum += number[j];
            j++;
        }
        n = j;
...
//最大隶属度最小值法
float Fuzzy_controller::defuzzSmallestOfMax(int u_index[], float u_u[])
{
    float step = fabs(umax - umin) / (2 * (N / 2));
    float num_index[9];
    float most = 0;
    ...
    nmin = *min_element(number, number + n);
    out = nmin;
    return out;
}

//最大隶属度法最大值法
float Fuzzy_controller::defuzzLargestOfMax(int u_index[], float u_u[])
{
    float step = fabs(umax - umin) / (2 * (N / 2));
    float num_index[9];
    float most = 0;
    ...
    nmax = *max_element(number, number + n);;
    out = nmax;
    return out;
}
...
float Fuzzy_controller::realize(float t, float a)
{
   ...
   //j = 0;
    ...
    //y = defuzzMeanOfMax(u_index, u_u);
    //y = defuzzSmallestOfMax(u_index, u_u);
    //y = defuzzLargestOfMax(u_index, u_u);
	return y;
}
